...
//
// options : rtkpost [-t title][-i file][-r file][-b file][-nf file ...][-n file ...]
//                   [-d dir][-o file]
...
ProcessingThread::ProcessingThread(QObject *parent, int nneta):QThread(parent)
{
    n=stat=0;
    ...
    rov=base=NULL;
    nnet = nneta;
    for (int i=0;i<nnet+5;i++) {infile[i]=new char[1024];infile[i][0]='\0';};
    outfile[0]='\0';
...
{
    for (int i=0;i<nnet+5;i++) delete[] infile[i];
    if (rov) delete[] rov;
    if (base) delete[] base;
    rov=base=NULL;
}
    ...
    : QDialog(parent)
{
    ...
    NetFiles.resize(MNNET);
    //for (i=0;i<MNNET;i++) NetFiles.resize(MAXHIST);
    connect(BtnNetObs,SIGNAL(clicked(bool)),this,SLOT(BtnNetObsClick()));
...
void MainForm::showEvent(QShowEvent* event)
{
    ...
    QCommandLineOption netFileOption(QStringList() << "nf" << "netfile",
            QCoreApplication::translate("main", "rinex network stations <file>"),
            QCoreApplication::translate("main", "file"));
    parser.addOption(netFileOption);
    ...
    if (parser.isSet(netFileOption)) {
        QStringList files=parser.values(netFileOption);
        if(NetFiles.size()<files.size())
            NetFiles.resize(files.size());
        for (int n=0;n<files.size();n++)
            NetFiles[n].prepend(files.at(n));
    }
...
// callback on button-network-observation-files------------------------------
void MainForm::BtnNetObsClick()
{   bool ok;
    ...
    QString myqstr = NNetSta->text();
    int k = myqstr.toInt(&ok);
    if(ok){
        if (k>MNNET){
            k = MNNET;
            NNetSta->setText(QString("%1").arg(k));
            showmsg("Maximum allowed number of network stations is %i", k);
        }
        NetObsDialog * netObsDialog= new NetObsDialog(this, k, files);
        netObsDialog->exec();
    }else{
        showmsg("error : non-integer value for number of network stations");
    }
...
void MainForm::BtnExecClick()
{
    ...
    int nnet=1;
    bool ok;
    if (PMODE_NET==PosMode){
        nnet = NNetSta->text().toInt(&ok);
        if(ok){
            for(int j=0;j<nnet;j++)
                if(NetFiles[j].value(0)==""){
                    showmsg("error : one or more network station obs files are missing");
                    return;
                }
...
void MainForm::ExecProc(int nnet)
{
    ...
    ProcessingThread *thread= new ProcessingThread(this, nnet);
...
void MainForm::ReadNetList(QSettings *ini, const QString &key, int i)
{
    QString item;

    item=ini->value(QString("%1%2").arg(key).arg(i,3),"").toString();
    if (item!="") NetFiles[i].prepend(item);
}
void MainForm::ReadNetListHist(QSettings *ini, const QString &key, int i)
{
    QString item;
    for(int j=0;j<MAXHIST;j++){
        item=ini->value(QString("%1%2_%3").arg(key).arg(i,3).arg(j,3),"").toString();
        if (item!="") NetFiles[i].append(item);
    }    
}
void MainForm::ReadBaselineConst(QSettings *ini, const QString &key)
{
    for(int i=0; i<MNNET;i++){
        BaseLine[i*2]        =ini->value  (QString("%1%2%3").arg(key).arg("len").arg(i,3),  0.0).toDouble();
        BaseLine[i*2+1]        =ini->value  (QString("%1%2%3").arg(key).arg("sig").arg(i,3),  0.0).toDouble();
        BaseLineConst[i]      =ini->value(QString("%1%2%3").arg(key).arg("const").arg(i,3),  0).toInt();
    }
}
// write history ------------------------------------------------------------
...
void MainForm::WriteNetList(QSettings *ini, const QString &key, int i)
{ 
   ini->setValue(QString("%1%2").arg(key).arg(i,3),NetFiles[i].value(0));

}
void MainForm::WriteNetListHist(QSettings *ini, const QString &key, int i)
{
   for(int j=1;j<NetFiles[i].size();j++)
        ini->setValue(QString("%1%2_%3").arg(key).arg(i,3).arg(j,3),NetFiles[i].value(j));
}
// add history --------------------------------------------------------------
...
void MainForm::UpdateEnable(void)
{
    ...
    bool modern=PMODE_NET==PosMode;

    LabelInputFile1->setText(moder||modern?tr("RINEX OBS: Rover"):tr("RINEX OBS"));
    InputFile2     ->setEnabled(moder);
    BtnInputFile2  ->setEnabled(moder);
    BtnInputPlot2  ->setEnabled(moder);
    BtnInputView2  ->setEnabled(moder);
    NNetSta        ->setEnabled(modern);
    BtnNetObs        ->setEnabled(modern);
...
void MainForm::LoadOpt(void)
{
    ...
    NNetSta->setText(ini.value("set/nnetsta",     "1").toString());
    ...
    for(int i=0; i<MNNET;i++){
        ReadNetList(&ini, "set/netfile", i);
        ReadNetListHist(&ini, "hist/netfile", i);
    }
    ...
    ReadBaselineConst(&ini, "opt/baseline");
...
void MainForm::SaveOpt(void)
{
    ...
    int k = myqstr.toInt(&ok);
    if(ok)
        ini.setValue("set/nnetsta",      myqstr);
        for(int j=0;j<k;j++){
            WriteNetList(&ini,"set/netfile", j);
            WriteNetListHist(&ini,"hist/netfile", j);
